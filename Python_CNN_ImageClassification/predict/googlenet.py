...
train_generator = train_datagen.flow_from_directory('hand/train_720', target_size=(720, 720),  batch_size=4, class_mode='categorical')
#(X_train, Y_train) = train_datagen.flow_from_directory('hand/train_720', target_size=(720, 720), batch_size=4, class_mode='categorical')

test_datagen = ImageDataGenerator(rescale=1./255)
test_generator = test_datagen.flow_from_directory('hand/test_720', target_size=(720, 720), batch_size=3, class_mode='categorical')
...
if __name__ == "__main__":
    '''img = imageio.imread('hand/test_720/1/1_14.jpg', pilmode='RGB')
    img = np.array(Image.fromarray(img).resize((720, 720))).astype(np.float32)
    img[:, :, 0] -= 123.68
    img[:, :, 1] -= 116.779
    img[:, :, 2] -= 103.939
    img[:,:,[0,1,2]] = img[:,:,[2,1,0]]
    ...
    '''for e in range(24):
        batches = 0
        for X_batch, Y_batch in train_datagen.flow(X_train, Y_train, batch_size=4):
    ...
    model.save('googlenet_weights.h5')
